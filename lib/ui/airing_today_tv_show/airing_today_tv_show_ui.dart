...
class AiringTodayTvShowUi extends StatefulWidget {
  @override
  _AiringTodayTvShowUiState createState() => _AiringTodayTvShowUiState();
}

class _AiringTodayTvShowUiState extends State<AiringTodayTvShowUi>
    with AutomaticKeepAliveClientMixin {
  final _airingTodayTvShowBloc =
      kiwi.Container().resolve<AiringTodayTvShowBloc>();
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return BlocProvider(
      create: (context) => _airingTodayTvShowBloc,
  ...
  void initState() {
    page = 1;
    _airingTodayTvShowBloc.onAirTvShowInitiated(page);
    super.initState();
  }

  @override
  void dispose() {
    _airingTodayTvShowBloc.close();
    super.dispose();
  }
  ...
  Scaffold _buildScaffold() {
    return Scaffold(
      ...
      body: BlocBuilder(
        bloc: _airingTodayTvShowBloc,
        builder: (context, AiringTodayTvShowState state) {
          if (state.isLoading) {
  ...
  Widget _nowPlayingMovies(AiringTodayTvShowState state) {
  ...
  bool _handleScroolNotification(ScrollNotification notification) {
    if (notification is ScrollEndNotification &&
      ...
      page++;
      _airingTodayTvShowBloc.onAirTvNextResult(page);
    }
    return false;
  }
