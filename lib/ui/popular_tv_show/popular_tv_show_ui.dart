...
class PopularTvShowUi extends StatefulWidget {
  @override
  _PopularTvShowUiState createState() => _PopularTvShowUiState();
}

class _PopularTvShowUiState extends State<PopularTvShowUi>
    with AutomaticKeepAliveClientMixin {
  final _popularTvShowBloc = kiwi.Container().resolve<PopularTvShowBloc>();
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return BlocProvider(
      create: (context) => _popularTvShowBloc,
  ...
  void initState() {
    page = 1;
    _popularTvShowBloc.onPopularTvShowInitiated(page);
    super.initState();
  }

  @override
  void dispose() {
    _popularTvShowBloc.close();
    super.dispose();
  }
  ...
  Scaffold _buildScaffold() {
    return Scaffold(
      ...
      body: BlocBuilder(
        bloc: _popularTvShowBloc,
        builder: (context, PopularTvShowState state) {
          if (state.isLoading) {
  ...
  Widget _nowPlayingMovies(PopularTvShowState state) {
  ...
  bool _handleScroolNotification(ScrollNotification notification) {
    if (notification is ScrollEndNotification &&
      ...
      page++;
      _popularTvShowBloc.onPopularTvNextResult(page);
    }
    return false;
  }
